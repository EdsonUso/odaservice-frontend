...
const connectSkill = (event) => {
    event.preventDefault();
    const elementUri = document.querySelector('#uri');
    const elementChannel = document.querySelector('#channelId');
    const URI = elementUri.value;
    const channelId = elementChannel.value;
    if (URI && channelId) {
        Bots.connect({
            URI,
            channelId
    ...
    elementUri.value = '';
    elementChannel.value = '';
    return false;
};

const form = document.querySelector('#config-form');
form.addEventListener('submit', connectSkill);
